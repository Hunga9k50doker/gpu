...
class ClientAPI {
  ...
  async makeRequest(
    ...
    // token_cookie_session=nwodtw2es44gu3ulzff4ck6fqvyv4eag.yHyLVmwU5syt9AIVEVAqvBJS5TW%2Bq7JFy2FQb0fhekI%3D; token_cookie_session_domain=nwodtw2es44gu3ulzff4ck6fqvyv4eag.yHyLVmwU5syt9AIVEVAqvBJS5TW%2Bq7JFy2FQb0fhekI%3D; twitter_oauth_state=X9FTTm7xJzW-VQRAespqIi6-skSNoQ2r0-X6awVtOHA; twitter_code_verifier=13bh2UFfEPRzuZBNOm0rnxLxaxq73GdXpKiau3zs39A
  ...
  async auth(retries = 5) {
    const url = `${this.baseURL}/auth/eth/verify`;
    const result = await this.getNonce();
    if (!result) {
      this.log("Can't get nonce", "error");
      return null;
    }
    const { nonce, cookie } = result;
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        return this.getCookieData(setCookie);
      } catch (error) {
  ...
  async getNonce(retries = 5) {
    const url = `${this.baseURL}/auth/eth/nonce`;
    let agent = null;
    if (this.proxy && settings.USE_PROXY) {
      agent = new HttpsProxyAgent(this.proxy);
    }
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        return { nonce: response.data, cookie };
      } catch (error) {
        console.log(error);
        if (attempt < retries - 1) {
          await sleep(5);
  ...
  async sendAPI() {
    return this.makeRequest(`https://support-umami.ygu6ir.easypanel.host/api/send`, "post", {
      type: "event",
      payload: {
        website: "59d36b8f-8ed5-4a62-8813-2feec176ee63",
        screen: "1536x864",
        language: "en-US",
        title: "Road to TGE - $GPU Drop is Coming | GPU.net",
        hostname: "token.gpu.net",
        url: `https://token.gpu.net/?ref=${settings.REF_CODE}`,
        referrer: "",
      },
    });
  }
  ...
  async checkin() {
    return this.makeRequest(`${this.baseURL}/users/streak`, "post");
  }

  async claimMulti(category) {
    return this.makeRequest(`${this.baseURL}/users/quests/${category}/multiplexer`, "get");
  }
  ...
  //   return this.makeRequest(`${this.baseURL}/users/social/tasks`, "get");
  // }

  async getTasks(type) {
    return this.makeRequest(`${this.baseURL}/users/${type}/tasks`, "get");
  }

  async completeTask(type, id, method = "post") {
    return this.makeRequest(`${this.baseURL}/users/${type}/tasks/${id}/verify`, method);
  }

  async handleCheckin(userData) {
    const lastChekin = Math.floor(new Date(userData?.lastVisitDate).getTime() / 1000);
    const nextMiningTime = lastChekin + 24 * 60 * 60;
    const nextDate = new Date(nextMiningTime * 1000);
    const dateNow = new Date();

    if (dateNow > nextDate || !userData?.lastVisitDate) {
      this.log(`Starting checkin...`, "info");
      const result = await this.checkin();
      if (!result.success) {
        return this.log(`Mining failed: ${JSON.stringify(result.error || {})}`, "error");
      }
      this.log(`Checkin success!`, "success");
    } else {
      this.log(`Last mining time: ${new Date(lastChekin * 1000).toLocaleString()}`, "warning");
    }
  }

  async handleTasks() {
    const types = settings.TASKS_ID;

    for (const type of types) {
      await sleep(1);
      this.log(`Getting tasks ${type}...`);
      const tasks = await this.getTasks(type);
      if (!tasks.success) {
      ...
      const taskCompletd = [];
      const taskAvaliable = tasks.data.filter((item) => {
        if (!item.completed && !settings.SKIP_TASKS.includes(item.id)) return item;
        else taskCompletd.push(item);
      });

      for (const task of taskAvaliable) {
        const { id, name: title, experience } = task;
        ...
        const method = type == "gpunet" ? "post" : "get";
        const result = await this.completeTask(type, id, method);
        if (result.success && result?.data?.success) {
          this.log(`Task ${id} | ${title} completed successfully | Reward: ${experience} | ${JSON.stringify(result.data)}`, "success");
        } else {
          if (result.data?.message == "Task not verified") {
            this.log(`Task ${id} | ${title} failed, You need complete manually | Details: ${JSON.stringify(result || {})}`, "warning");
          } else {
      ...
      if (taskCompletd.length == tasks.data.length) {
        const claimResult = await this.claimMulti(type);
        console.log(claimResult);
        if (claimResult?.data?.success) {
          this.log(`Multiplier ${colors.green("Is Claimed")}`, "success");
        } else {
          this.log(`Multiplier ${colors.red("Not Claimed")} | ${JSON.stringify(claimResult)}`, "warning");
        }
      }
    }
  }
  ...
  getCookieData(setCookie) {
    try {
      ...
      // const item =
      const nonceMatch = item.match(/nonce=([^;]+)/);
      const nonceDomain = item.match(/nonce_domain=([^;]+)/);
      const tokenSession = item.match(/token_cookie_session=([^;]+)/);
      const tokenSessionDomain = item.match(/token_cookie_session_domain=([^;]+)/);
      if (nonceMatch && nonceMatch[0]) {
        cookie.push(nonceMatch[0]);
      }
      if (nonceDomain && nonceDomain[0]) {
        cookie.push(nonceDomain[0]);
      }
      if (tokenSession && tokenSession[0]) {
        cookie.push(tokenSession[0]);
      }
      if (tokenSessionDomain && tokenSessionDomain[0]) {
        cookie.push(tokenSessionDomain[0]);
      }
  ...
  async handleSyncData() {
    ...
    if (userData.success) {
      let points = balance.data;
      const { referralCode, isTwitterConnected, currentStreak, referralsCount, email } = userData.data;
      this.log(`[${referralCode}] Ref count: ${referralsCount} | Twitter: ${isTwitterConnected ? "Connected" : "Not set"} | Streak login: ${currentStreak} | Points: ${points || 0}`, "custom");
    } else {
  ...
  async runAccount() {
    ...
    if (userData.success) {
      if (userData.data?.isTwitterConnected) {
